#ifndef GPCOCTOMAP_GP_OCTOMAP_H
#define GPCOCTOMAP_GP_OCTOMAP_H
...
namespace gpcoctomap {
    ...
    public:
         ...
         */
        GPCOctoMap(float resolution, unsigned short block_depth, float sf2, float ell, float noise, float l,
                  float min_var,
        ...
        /*
         * @brief Insert PCL PointCloud into GPCOctoMaps.
        ...
        class RayCaster {
        public:
            RayCaster(const GPCOctoMap *map, const point3f &start, const point3f &end) : map(map) {
                assert(map != nullptr);
        ...
        public:
            LeafIterator(const GPCOctoMap *map) {
                assert(map != nullptr);
