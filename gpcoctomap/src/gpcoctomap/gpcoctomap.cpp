...
namespace gpcoctomap {
    ...
    GPCOctoMap::GPCOctoMap() : GPCOctoMap(0.1f, 4, 1.0, 1.0, 0.01, 100, 0.001f, 1000.0f, 0.02f, 0.3f, 0.7f) { }

    GPCOctoMap::GPCOctoMap(float resolution, unsigned short block_depth, float sf2, float ell, float noise, float l,
                         float min_var,
    ...
    void GPCOctoMap::set_resolution(float resolution) {
    ...
    void GPCOctoMap::set_block_depth(unsigned short max_depth) {
...
#pragma omp parallel for
    ...
    void GPCOctoMap::get_bbox(point3f &lim_min, point3f &lim_max) const {
    ...
    void GPCOctoMap::get_blocks_in_bbox(const point3f &lim_min, const point3f &lim_max,
    ...
    int GPCOctoMap::has_gp_points_in_bbox(const BlockHashKey &key) {
        point3f half_size(block_size / 2.0f, block_size / 2.0f, block_size / 2.0);
        point3f lim_min = hash_key_to_block(key) - half_size;
        point3f lim_max = hash_key_to_block(key) + half_size;
    ...
    int GPCOctoMap::has_gp_points_in_bbox(const point3f &lim_min,
    ...
    int GPCOctoMap::has_gp_points_in_bbox(const ExtendedBlock &block) {
    ...
    Block *GPCOctoMap::search(BlockHashKey key) const {
        auto block = block_arr.find(key);
        if (block == block_arr.end()) {
            return nullptr;
        } else {
    ...
    OcTreeNode GPCOctoMap::search(point3f p) const {
    ...
    OcTreeNode GPCOctoMap::search(float x, float y, float z) const {
